...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuetemprature != cur->valuetemprature)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuetemprature Changed\n");
            ret = 1;
        }
    }
    if(last->valuesoil_h != cur->valuesoil_h)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuesoil_h Changed\n");
            ret = 1;
        }
    }
    if(last->valuesunlight != cur->valuesunlight)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valuesunlight Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valuetemprature = exchangeBytes(gizY2XFloat(temprature_RATIO,  temprature_ADDITION, dataPoints->valuetemprature));     
    devStatusPtr->valuesoil_h = exchangeBytes(gizY2X(soil_h_RATIO,  soil_h_ADDITION, dataPoints->valuesoil_h)); 
    devStatusPtr->valuesunlight = exchangeBytes(gizY2X(sunlight_RATIO,  sunlight_ADDITION, dataPoints->valuesunlight)); 
